import { sock, playerMe, chatFocused } from "./client.js";
import {
  FPS,
  LASER_SPD,
  SHIP_SIZE,
...
function drawScene(players) {
  ...
  ctx.setTransform(
    ...
    0,
    1,
    -(players[playerMe.id].x - CANVAS_WIDHT / 2),
    -(players[playerMe.id].y - CANVAS_HEIGHT / 2)
  );

  //draw minimap
  ctx.strokeStyle = "white";
  ctx.strokeRect(
    players[playerMe.id].x + MINI_MAP_X_OFF,
    players[playerMe.id].y + MINI_MAP_Y_OFF,
    145,
    144
  ); // last two numbers dictate end of map x/y direction
  ...
  ctx.fillText(
    `Your current score: ${players[playerMe.id].shipsDestroyed}`,
    players[playerMe.id].x + SCORE_OFF_X,
    players[playerMe.id].y - SCORE_OFF_Y
  );

  //write current leader's score
  if (leader) {
    ctx.fillText(
      `Leader is ${players[leader].name} with ${players[leader].shipsDestroyed}`,
      players[playerMe.id].x - LEADER_OFF_X,
      players[playerMe.id].y - LEADER_OFF_Y
    );
  }
}
  ...
  function shootLaser() {
    if (shootTimer === 0) {
      spaceship.laser = {
        ...
        dy:
          -1 *
          (LASER_SPD + Math.abs(spaceship.thrust.y)) *
          Math.sin(spaceship.a),
      };
